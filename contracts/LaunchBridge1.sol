  ...
  PausableUpgradeable
{
  ...
  function totalUSDBalanceNoUpdate() public view returns (uint256) {
    ...
    uint256 chi = _rmul(
      _rpow(pot.dsr(), block.timestamp - pot.rho(), _RAY),
      pot.chi()
    );
    return
      DAI.balanceOf(address(this)) +
      _rmul(DSR_MANAGER.pieOf(address(this)), chi);
  }
  ...
  function _recordDepositETH(
    ...
    uint256 sharesToIssue = (depositedAmount * totalETHShares) /
      _totalETHBalance;
    if (sharesToIssue == 0) {
